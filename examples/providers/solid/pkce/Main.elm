...
main =
    application
        ...
        , view =
            view
                { title = "Pods Solid Community AU - Flow: Authorization Code w/ PKCE"
                , btnClass = class "btn-solid"
                }
        }
...
hence the user info endpoint and Turtle decoder

-}
configuration : Configuration
configuration =
    { authorizationEndpoint =
        { defaultHttpsUrl | host = "pods.solidcommunity.au", path = "/.oidc/auth" }
    , tokenEndpoint =
        { defaultHttpsUrl | host = "pods.solidcommunity.au", path = "/.oidc/token" }
    , userInfoEndpoint =
    ...
    , clientId =
        "https://your_server/client-id.json"
    , scope =
...
getUserInfo { userInfoEndpoint } token =
    Http.request
        { method = "GET"
        , body = Http.emptyBody
        , headers = OAuth.useToken token []
        , url = Url.toString userInfoEndpoint
        , expect = Http.expectString GotUserInfo
        , timeout = Nothing
        , tracker = Nothing
        }
...
viewUserInfo { btnClass } user =
    [ div [ class "flex", class "flex-column" ]
        [ p [] [ text (Rdf.toUrl user.topic) ]
        , div []
            [ button
